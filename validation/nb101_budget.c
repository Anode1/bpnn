/* nb101_budget.c -- NAS-Bench-101 FULL archive -> per-run accuracies AT ONE TRAINING BUDGET.
 *
 * WHY, and it is a different question from the one nb101_trials.c answers. The full archive holds three
 * runs at each of four training budgets: 4, 12, 36 and 108 epochs. Multi-fidelity and early-stopping
 * search, which is standard practice, trains candidates briefly and ranks them on that, assuming the
 * cheap ranking transfers. If run-to-run noise is much larger at 4 epochs than at 108, then those
 * methods select on a far noisier signal than anyone reports, and the flip rates at low budget are the
 * number that matters to them.
 *
 * nb101_trials.c keys records by architecture hash ALONE, because the epoch-108 archive has only one
 * budget and the field is skipped. Pointed at the full archive that quietly appends all four budgets
 * into one architecture's trial list, mixing 4-epoch and 108-epoch runs as if they were replicates. This
 * program parses the budget and keeps only the requested one.
 *
 * usage: ./nb101_budget nasbench_full.tfrecord <4|12|36|108> out_triples.txt
 *
 * (adapted from nb101_trials.c, whose header follows)
...
{
    const char *inpath  = argc > 1 ? argv[1] : "nasbench_full.tfrecord";
    int         want_budget = argc > 2 ? atoi(argv[2]) : 108;
    const char *outpath = argc > 3 ? argv[3] : "validation/nb101_b108.txt";
    ...
    for (;;) {
        ...
        /* the budget sits between the hash and the adjacency string; nb101_trials.c skips it */
        { int bud = 0;
          while (p < len && (pay[p] == ',' || pay[p] == ' ')) p++;
          while (p < len && pay[p] >= '0' && pay[p] <= '9') bud = bud*10 + (pay[p++] - '0');
          if (bud != want_budget) continue; }
    ...
    fprintf(out, "# NAS-Bench-101 at %d epochs: one architecture per line, three per-run "
                 "validation accuracies in percentage points\n", want_budget);
    {
        uint32_t s; long written = 0;
        for (s = 0; s < TSIZE; s++) {
            int k;
            if (!used[s] || tab[s].cnt != 3) continue;
            for (k = 0; k < 3; k++)
                fprintf(out, "%s%.4f", k ? " " : "", 100.0 * tab[s].v[k]);
            fprintf(out, "\n");
        written++;
        }
        fclose(out);
